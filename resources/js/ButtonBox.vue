    ...
    <div class="button_box-fieldtype-wrapper flex -mx-2">
        ...
        >
            <button
                ...
                :class="option.value === value ? 'border-primary border-2 rounded p-px' : 'border-2 rounded border-white p-px'"
            >
                <span v-if="!option.image.length > 0 && !option.icon">{{ option.label || option.value }}</span>

                <button_box-image v-if="option.image.length > 0" :id="option.image[0]" />
            </button>
            <button
                v-else
                :id="name + $index"
